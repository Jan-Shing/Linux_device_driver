...
static unsigned int globalmem_poll(struct file *filp, poll_table *wait)
{
	...
	poll_wait(filp, &dev->r_wait, wait); /* Add to Read wait Queue*/
	poll_wait(filp, &dev->w_wait, wait); /* Add to Write wait Queue*/
	...
	mutex_unlock(&dev->mutex);
	printk(KERN_INFO "call driver poll mask is %d\n",mask);
	return mask;
}
